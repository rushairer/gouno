...
func setupRoutes() {
    ...
    // 全局限频：每分钟 100 次
    r.Use(middleware.RateLimitMiddleware(ctx, 100, time.Minute))
    ...
    api := r.Group("/api/v1")
    {
        ...
        auth := api.Group("/auth")
        auth.Use(middleware.RateLimitMiddleware(ctx, 10, time.Minute))
        {
        ...
        public.Use(middleware.RateLimitMiddleware(ctx, 200, time.Minute))
        {
...
#### `IPRateLimitMiddleware(ctx context.Context) gin.HandlerFunc`
创建默认的 IP 限频中间件（每分钟 60 次）。
`ctx` 用于控制后台清理 goroutine 的生命周期。

#### `RateLimitMiddleware(ctx context.Context, limit int, window time.Duration) gin.HandlerFunc`
创建自定义配置的限频中间件。

**参数：**
- `ctx`：用于控制后台清理 goroutine 的生命周期，取消后自动退出
...
```go
// 每分钟 30 次
middleware.RateLimitMiddleware(ctx, 30, time.Minute)

// 每小时 1000 次
middleware.RateLimitMiddleware(ctx, 1000, time.Hour)

// 每秒 10 次
middleware.RateLimitMiddleware(ctx, 10, time.Second)
```
...
func CustomIPRateLimitMiddleware() gin.HandlerFunc {
    ctx := context.Background()
    limiter := middleware.NewRateLimiter(ctx, 60, time.Minute)
...
func UserRateLimitMiddleware() gin.HandlerFunc {
    ctx := context.Background()
    limiter := middleware.NewRateLimiter(ctx, 100, time.Minute)
